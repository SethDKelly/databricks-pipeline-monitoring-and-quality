# DMTZ Implementation Program

**Status:** BLOCKED — CKR-A–CKR-F COMPLETE / CKR-G NEXT

**ADF status mirror: COMPLETE ADF-A–ADF-H; ADF-EX-17 DEFERRED VERIFICATION; FOUNDATION EXIT ACCEPTED; IMPLEMENTATION 001-A NEXT.**

**CKR status mirror: COMPLETE CKR-A–CKR-F; NEXT CKR-G; IMPLEMENTATION 001-A BLOCKED ON CKR EXIT.**

Product implementation remains blocked while CKR completes documentation-authority migration.

Current routing uses the ownership inventory. Foundation/glossary, concepts/SYN, authority vocabulary, REF, AUTH, HLTH and OPS resolve to canonical owners. Phase 001–007 are provenance for migrated meaning. EXPL/INTG/ARCH retain assigned legacy owners.

Current work:

- CKR-A–F — COMPLETE / ACCEPTED;
- CKR-G — NEXT / READY / NOT STARTED;
- Implementation 001-A — NOT ACTIVE / BLOCKED until CKR-K.

Accepted ranges remain SYN-001–035, REF-001–030, AUTH-001–053, HLTH-001–066, OPS-001–123, EXPL-001–160, INTG-001–270 and ARCH-001–500.
