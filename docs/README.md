...
**DPTN state:** DPTN-A IN EXECUTION — IMPLEMENTATION 001-A BLOCKED / NOT STARTED ON DPTN EXIT.
...
DPTN-A is planning a physical topology normalization only. Proposed future destinations in [`documentation_topology_normalization/move_map.json`](documentation_topology_normalization/move_map.json) are not current semantic owners. No documentation subtree moves in DPTN-A.
...
The catalog contains **24 accepted concepts**. ADF and CKR exits are accepted. **DPTN-A is IN EXECUTION and Implementation 001-A is BLOCKED / NOT STARTED until DPTN-G exit acceptance.**
...
4. Do not substitute a DPTN-A planned target path for the current owner before a later accepted cutover.
...
- DPTN-A's move map is planning evidence only and cannot create a second current owner.
