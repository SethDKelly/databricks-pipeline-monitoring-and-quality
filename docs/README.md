...
This file remains the **living authority for completed design-phase progression**. Discovery and progression are separate responsibilities; current semantic lookup remains ownership-oriented rather than chronology-oriented.

**CKR state:** CKR-A–CKR-K COMPLETE / ACCEPTED — CKR EXIT ACCEPTED.

**DPTN state:** DPTN-A–G COMPLETE / ACCEPTED — DPTN EXIT ACCEPTED.

**Implementation state:** Implementation 001-A NEXT / READY / NOT STARTED.

## Documentation authority architecture

Current semantic ownership is resolved through [`canonical_knowledge_retrofit/canonical_ownership_inventory.json`](canonical_knowledge_retrofit/canonical_ownership_inventory.json). The normalized topology uses first-class `docs/<family>/` semantic-owner roots, [`index.md`](index.md) as the authored discovery root, and top-level `knowledge/` as deterministic generated OKF v0.2 compatibility output.

Completed CKR evidence lives under [`history/retrofits/ckr/`](history/retrofits/ckr/), completed ADF evidence under [`history/foundations/adf/`](history/foundations/adf/), the pre-convergence authored OKF tree under [`history/routing/okf-pre-dptn-e/`](history/routing/okf-pre-dptn-e/), and the completed DPTN program under [`history/retrofits/dptn/`](history/retrofits/dptn/). All are provenance only.

> **A current semantic question resolves to one current owner. History, generated routing, path vacancy and prior compatibility paths never establish current meaning independently of the ownership ledger.**
...
The catalog contains **24 accepted concepts**. ADF, CKR and DPTN exits are accepted. **Implementation 001-A is NEXT / READY / NOT STARTED.**
...
- **DPTN-G — Conservation Audit, Legacy-Path Retirement & Exit Review: COMPLETE / ACCEPTED.**
- **DPTN EXIT: ACCEPTED.**

The complete DPTN authority/move/review/fixture record is historical provenance at [`history/retrofits/dptn/README.md`](history/retrofits/dptn/README.md). DPTN is no longer a live routing layer.
...
5. Current agent/rule/reference routing uses first-class semantic-owner paths.
...
- The durable final documentation-topology guard is `scripts/agentic/validate_documentation_topology.py`; DPTN phase tooling is historical.
- Implementation progression lives in `implementation/README.md`.
- `docs/history/` is explicitly non-current.

**Implementation 001-A remains NOT STARTED until explicitly selected by the human.**
