...
def main():
    ...
    lines+=['','## Knowledge lifecycle','',f'- Deprecated knowledge entries: **{deprecated}**',f'- Stale knowledge entries: **{stale}**','','## Notes','','- Provider tool-in-the-loop runtime verification remains independent per tool; ADF-EX-17 is deferred under the bounded progression exception.','- CKR changes documentation authority/routing, not accepted DMTZ semantics; Implementation 001-A remains blocked until CKR exit.','- CKR-B protects the canonical foundation/glossary cutover.','- CKR-C protects the 24-concept catalog and SYN-001–SYN-035.','- CKR-D protects REF-001–REF-030, AUTH-001–AUTH-053 and the authority vocabulary from omission, partial cutover and authority/evidence collapse.','- `docs/canonical/` becomes current semantic authority only through the CKR inventory and atomic cutover contract.','- Databricks vendor skills remain reviewed operational dependencies, not semantic authority.','- Agentic secret scanning is a high-confidence repository guard, not organization-wide secret scanning.','']
    report='\n'.join(lines)
    if args.report:
