#!/usr/bin/env python3
"""Report generated OKF routing entries affected by changed current resources.

RESOURCE and BODY-LINK relationships are review-impact hints only. A candidate is
not automatically stale and this helper never changes current DMTZ semantics.
"""
...
def add(reverse,target,concept,role):
    reverse.setdefault(target,[]);item=(concept,role)
    if item not in reverse[target]:reverse[target].append(item)

def under_current_owner(target:Path,repo:Path,current_owner_roots:list[str])->bool:
    if not target.is_relative_to(repo):return False
    rel=target.relative_to(repo).as_posix()
    return any(rel==root or rel.startswith(root+'/') for root in current_owner_roots)
...
def main():
    ...
    inventory=json.loads((repo/'docs/canonical_knowledge_retrofit/canonical_ownership_inventory.json').read_text(encoding='utf-8'))
    canonical_owner_roots=list(inventory.get('canonical_owner_roots',[]))
    if not canonical_owner_roots:raise SystemExit('ERROR CKR ownership inventory has no canonical_owner_roots')
    for concept in sorted((repo/'knowledge').rglob('*.md')):
        ...
        for raw in LINK_RE.findall(text):
            ...
            if target is not None and under_current_owner(target,repo,canonical_owner_roots):add(reverse,target,concept,'BODY-LINK')
    if a.all:
    ...
    for raw in a.changed:
        ...
        if not items:print('  no generated OKF RESOURCE/BODY-LINK routes; no automatic projection edit required')
