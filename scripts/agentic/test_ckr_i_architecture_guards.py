...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix='dmtz-ckri-guards-') as td:
        ...
        mutate(repo, 'docs/canonical_knowledge_retrofit/ckr_i_semantic_conservation_matrix.md', lambda t: t.replace('name equality/timestamp proximity ≠ exact identity/deployment/run/input/output join', 'name equality/timestamp proximity = exact identity/deployment/run/input/output join'), 'timestamp/name join inference regression', errors)
